...
int newfs_driver_write(int offset, uint8_t *in_content, int size)
{
    ...
    uint8_t *temp_content = (uint8_t *)malloc(size_aligned);
    uint8_t *cur = temp_content;
    newfs_driver_read(offset_aligned, temp_content, size_aligned);//读出需要的磁盘块到内存
    memcpy(temp_content + bias, in_content, size);//在内存覆盖指定内容

    ddriver_seek(NFS_DRIVER(), offset_aligned, SEEK_SET);
    while (size_aligned != 0)//将读出的磁盘块再依次写回到内存
    {
...
int newfs_sync_inode(struct newfs_inode *inode)
{
    ...
    if (NFS_IS_DIR(inode))/* 如果当前inode是目录，那么数据是目录项，且目录项的inode也要写回 */
    {
        ...
        while (dentry_cursor != NULL)
        {
            ...
            {
                newfs_sync_inode(dentry_cursor->inode);//递归调用
            }
            ...
            {
                write_length = 0;
                offset = NFS_DATA_OFS(inode->block_pointer[index++]);//下一个
...
int newfs_umount()
{
    ...
    //超级快刷回
    newfs_super_d.magic_num = NFS_MAGIC_NUM; /* 建立 in-disk 结构 */
